...
module.exports = function(source) {
  ...
  const processManifest = manifestProcessor(manifest, query);

  const callback = this.async();
  return processManifest(callback);
}

function manifestProcessor(manifest, query) {
  return function(callback) {
    if(!callback) {
      return processManifestSync(manifest, query);
    } else {
...
function processManifestAsync(manifest, query, callback) {
  fs.readFile(packageJSONPath, 'utf8', (err, data) => {
    ...
    const alteredManifest = assignValues(manifest, packageJSON, query);
    callback(err, JSON.stringify(alteredManifest, null, '\t'));
  });
}
